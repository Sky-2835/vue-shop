<template>
  <div >
    ...
    <el-form :model="addGoodsForm2" :rules="addGoodsForm2Rules" ref="addGoodsForm2" label-width="100px" label-position="top" class="form">
      <el-tabs v-model="activeName" @tab-click="handleTabClick" tabPosition='left' :before-leave='changeTabs'>
        ...
        <el-tab-pane label="基本信息" name="0">
             <el-form-item label="商品名称" prop="goods_name">
                <el-input v-model="addGoodsForm2.goods_name"></el-input>
             </el-form-item>
             <el-form-item label="商品价格" prop="goods_price">
                <el-input v-model="addGoodsForm2.goods_price" type='number' ></el-input>
             </el-form-item>
             <el-form-item label="商品重量" prop="goods_weight">
                <el-input v-model="addGoodsForm2.goods_weight" type='number'></el-input>
             </el-form-item>
             <el-form-item label="商品数量" prop="goods_number">
                <el-input v-model="addGoodsForm2.goods_number" type='number'></el-input>
             </el-form-item>
             <el-form-item label="商品分类(只能选择三级分类)" prop="goods_cat"> <!-- 写了校验规则，Form表单也无法校验级联选择器；只是为了在 label前面加上 * 号 -->
                  <el-cascader v-model="addGoodsForm2.goods_cat" :options="allCateDate" :props="cascaderConfig3" @change="cascaderChange3" clearable></el-cascader>
             </el-form-item>
        </el-tab-pane>
        <el-tab-pane label="商品参数" name="1">
            <!-- 为了保证用同一个 整个Form表单 多选框也要被 el-form-item 包裹-->
                ...
                <!-- 显示 这类商品即亲爸爸分类 第三级分类下所有商品的 全部共同属性 默认全选  通过请求商品分类获得数据  -->
                <!-- <div>所有属性</div>  -->
            <el-form-item :label='item.attr_name' v-for="item in manyData" :key="item.attr_id">
                ...
                <!-- 显示 此商品勾选的 改变的 属性   通过请求 (查询商品)商品所有信息 获得数据 -->
                <!-- <div>已选属性</div>  -->
                <!-- 跟上面一样 弄成多选框 ；但因服务器不会修改 属性 参数  返回值是一样的 ，即更改不了 。所以不写了-->
       <!-- <el-form-item> ....  </el-form-item>    -->   
        ...
        <el-tab-pane label="商品内容" name="4">  <!-- 富文本内添加的数据:以字符串保存出来  "<p> innerHtml </p> " -->
            ...
            <quill-editor ref="myQuillEditor" v-model="addGoodsForm2.goods_introduce" />
            <el-button type='primary' class="addBtn" @click="add">更新商品</el-button>
        </el-tab-pane>
      </el-tabs>
    </el-form>
     </el-card>
...
export default {
  data() { 
    ...
    /*   自己想的 当公用变量 依次校验名称 ，价格
      ...
      manyData:[],
      chooseManyData:[], // 商品勾选的 改变的 属性 
      
      // 静态参数 数据
      onlyData:[],
      chooseOnlyData:[],  // 商品勾选的 改变的 属性 
  ...
  methods:{
     // 根据商品ID 获取所有商品信息
async getGoodsAllInfo(){
    const {data : res } = await this.$axios.get(`goods/${this.$route.query.goods_id}`)
        if(res.meta.status !== 200) return this.$message.error('获取商品数据失败')
        let arr = res.data.goods_cat.split(',')  
        res.data.goods_cat = arr.map(item=>{
           return item-0
        })

        /* 这是更改后 重新请求来的 商品所有 属性 参数 信息；即已选的属性 参数。
        res.data.attrs.forEach(item=>{
          if(item.attr_sel == 'many'){
            ...
            const obj = {
             ...
             this.chooseOnlyData.push(obj)
          }
        }) */

        console.log(this.chooseManyData);
        console.log(this.chooseOnlyData);
        res.data.attrs = []
        res.data.pics = []
        this.addGoodsForm2 =  res.data
...
async handleTabClick(){ 
     ...
     cascaderChange3(){
        if(this.addGoodsForm2.goods_cat.length !== 3){
          this.addGoodsForm2.goods_cat = []
        }
     ...
     changeTabs(newActiveName,oldActiveName){
       if(oldActiveName === '0' && this.addGoodsForm2.goods_cat.length !== 3 ){
       ...
       if(newActiveName ==='3'){
         console.log(this.manyData);
         console.log(this.onlyData);
         console.log(this.addGoodsForm2.goods_introduce);
       }
         
  /*   只验证部分表单 验证完的"错误信息"   默认传入后面回调函数当实参
        自己想的 依次校验 名称 价格; 校验失败 不会切换tabs标签
        this.$refs.addGoodsForm2.validateField(['goods_name', 'goods_price'],(a)=>{
         if(a){
           this.$message.error('输入商品名称')
           this.format = false
           return 
         }
            this.$refs.addGoodsForm2.validateField('goods_price',a=>{
              if(a){
      ...
      handleRemove(file){
      /*  1 直接用filter方法
          this.addGoodsForm2.pics = this.addGoodsForm2.pics.    filter(item=>{
               return file.response.data.tmp_path !==     item.pic
            }) 2 用findIndex找出位置，再用splice切除 */
           const index = this.addGoodsForm2.pics.findIndex((item)=>{
                  return file.response.data.tmp_path === item.pic})  
           this.addGoodsForm2.pics.splice(index,1)
      },
  ...
  /*  !!!  服务器响应返回的对象{data:……  ,  meta:……}它被赋值 给了文件列表中文件对象的response属性 (注意 它跟axios请求返回的对象不同 {data:{data:…… ，meta:……}})
   成功跟失败 取决于文件列表中这个文件对象中的 status属性。
      ...
      issuccess(res,file){    //文件上传成功时 默认传参1服务器响应返回的对象 2文件对象 3文件列表
        ...
        /* let obj = {...res.data}
           delete obj.url
           this.addGoodsForm2.pics.push(obj) ↓或者*/
           const obj = {pic:res.data.tmp_path}
           this.addGoodsForm2.pics.push(obj)
           console.log(this.addGoodsForm2.pics);
      ...
      add(){
          //先对所有表单进行预验证
          this.$refs.addGoodsForm2.validate(async valid=>{
          ...
          不点击 这2个一定是空数组，但是有可能本身就为空。咋办呢。设置个变量计数*/
            ...
            this.manyData.forEach(item=>{
              ...
              this.addGoodsForm2.attrs.push(obj)
            }) 
          ...
          /* this.addGoodsForm2.attrs = [...this.manyData,...this.onlyData] */

          /* 不能直接将这个数组转成字符串再赋值给原数组，因为级联双向绑定的必须是一个数组，变成字符串了 要报错。所以深拷贝整个对象再操作，用作发送请求
            this.addGoodsForm2.goods_cat = this.addGoodsForm2.goods_cat.join(',') */
            const addGoodsForm3 = JSON.parse(JSON.stringify(this.addGoodsForm2))
            addGoodsForm3.goods_cat = addGoodsForm3.goods_cat.join(',')
            
            const addGoodsForm4 = {
                goods_name:this.addGoodsForm2.goods_name,
                goods_price:this.addGoodsForm2.goods_price,
                goods_number:this.addGoodsForm2.goods_number,
                goods_weight:this.addGoodsForm2.goods_weight,
                goods_introduce:this.addGoodsForm2.goods_introduce,
                pics:this.addGoodsForm2.pics,
                attrs:this.addGoodsForm2.attrs,
                goods_cat:addGoodsForm3.goods_cat,
            }
            
            console.log(addGoodsForm3.goods_cat);
            console.log(8888888888888);
            console.log(addGoodsForm4);

         // 发送添加商品的请求 。注意 商品名称要唯一不然会失败
         const {data : res } = await this.$axios.put(`goods/${this.$route.query.goods_id}`,addGoodsForm4)
  ...
  computed:{
    ...
    getID(){
       if(this.addGoodsForm2.goods_cat.length ===3){
         return this.addGoodsForm2.goods_cat[2]
       }
       return null
    }
  }
